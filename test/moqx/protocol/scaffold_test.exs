defmodule MOQX.Protocol.ScaffoldTest do
  ...
  test "resolves every built-in protocol explicitly" do
    assert Resolver.ids() == [:draft_18, :moq_lite_05]

    assert {:ok, MOQX.Protocol.Draft18} = Resolver.fetch(:draft_18)
    assert {:ok, MOQX.Protocol.MOQLite05} = Resolver.fetch(:moq_lite_05)

    assert {:error, :unknown_protocol} = Resolver.fetch(:moq_lite_04)
    assert {:error, :unknown_protocol} = Resolver.fetch(:cloudflare_draft_14)
    assert {:error, :unknown_protocol} = Resolver.fetch(:draft_16)
