defmodule Mix.Tasks.Moqx.Roundtrip do
  @moduledoc """
  End-to-end relay roundtrip test: connect publisher + subscriber, publish one
  frame, and verify the subscriber receives it.

  ## Usage

      mix moqx.roundtrip [relay_url] [options]

  Examples:

      mix moqx.roundtrip
      mix moqx.roundtrip https://interop-relay.cloudflare.mediaoverquic.com:443
      mix moqx.roundtrip --namespace moqx-e2e/demo --track demo --payload hello
  ...
  @shortdoc "Run a publisher/subscriber relay roundtrip smoke test"
  ...
  defp await_track_active!(track, namespace, track_name, timeout) do
    case MOQX.Helpers.await_track_active(track, timeout) do
      :ok ->
        :ok

      {:error, :timeout} ->
        Mix.raise("track activation timeout for #{namespace}/#{track_name}")

      {:error, %MOQX.RequestError{message: reason}} ->
        Mix.raise("track activation failed for #{namespace}/#{track_name}: #{inspect(reason)}")
    end
  end
