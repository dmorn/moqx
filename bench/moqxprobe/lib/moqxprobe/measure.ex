defmodule MOQXProbe.Measure do
  ...
  defp do_measure_moqx_stream_pressure(ctx, connection, config, handshake_latency_ms, collector) do
    ...
    {result, _ctx} =
      case config.stream_direction do
        "bidirectional" ->
    ...
    result = apply_transport_collector(result, collector_snapshot)
  ...
  defp apply_transport_collector(result, snapshot) do
    result
  ...
  defp measure_moqx_mixed_pressure(ctx, connection, config, handshake_latency_ms) do
    ...
    try do
      do_measure_moqx_mixed_pressure(ctx, connection, config, handshake_latency_ms, collector)
    after
      TransportTelemetryCollector.close(collector)
    end
  end

  defp do_measure_moqx_mixed_pressure(ctx, connection, config, handshake_latency_ms, collector) do
    application_started_at = monotonic_us()
    ...
    %{
      ...
      "summary" =>
        compact(%{
          ...
          "stream_send_bytes_accepted" => collector_snapshot.stream_send_bytes_accepted,
  ...
  defp send_paced_moqx_datagrams(
    ...
    ctx
    |> run_paced_datagram_sender(connection, config, count, started_at, rate, send_state)
    |> finish_paced_datagram_sender_result(config, count, started_at, send_state, ctx)
  end

  defp run_paced_datagram_sender(ctx, connection, config, count, started_at, rate, send_state) do
    DatagramSender.run(
      count: count,
      rate_per_second: rate,
      started_at_us: started_at,
      timeout: datagram_receive_timeout_ms(config) + 1_000,
      payload_mode: {:sequence_timestamp, config.datagram_size},
      ...
      max_queue_depth: datagram_paced_queue_depth(rate),
      send_fun: paced_datagram_send_fun(connection, config, count, started_at),
      transport_state: %{
        ctx: ctx,
        receive_state: Map.put(send_state, :traffic_sender, "datagram_sender"),
        accepted: 0,
        failure: nil
      },
      stop_on_error?: true
  ...
  defp finish_paced_datagram_sender_result(
         {:ok, snapshot},
         _config,
         _count,
         started_at,
  ...
  defp finish_paced_datagram_sender_result(
    ...
    ctx = transport_state[:ctx] || ctx
    failure = datagram_send_failure(config, reason, 1, count, 0)
    send_state = transport_state[:receive_state] || send_state

    {:error, failure, accepted, elapsed_ms(started_at), send_state, ctx}
  end
  ...
  defp datagram_paced_queue_depth(rate) do
    rate
    |> datagram_paced_burst_size()
    |> DatagramSender.default_max_queue_depth()
  end
  ...
  defp send_unidirectional_pressure_payloads(ctx, streams, payload, config, started_at) do
    ...
    with {:ok, sender} <-
      ...
      collect_send_only_events(
        ctx,
        sender,
        states,
        diagnostics,
        config,
        monotonic_us() + client_timeout_ms(config) * 1000
      )
    else
  ...
  defp collect_send_only_events(ctx, sender, states, diagnostics, config, deadline_us) do
    cond do
      all_stream_sends_complete?(states, config) ->
        {ctx, states} = finish_stream_pressure_sender(sender, ctx, states)
        send_only_result(ctx, states, config, nil, diagnostics)
  ...
  defp fail_send_only_stream(state, config, reason) do
    failure = send_only_stream_failure(state, config, reason)
  ...
  defp send_only_stream_failure(state, config, reason) do
    %{
      "phase" => "send_failed",
      "reason" => reason,
      "stream_index" => state.stream_state.index,
      ...
      "bytes_expected" => expected_stream_bytes(config),
      "bytes_received" => 0,
      "incomplete_bytes" => max(expected_stream_bytes(config) - state.stream_state.bytes_sent, 0),
      "send_completed" => state.send_completed,
      "send_cancelled" => state.send_cancelled
    }
  end
  ...
  defp send_only_result(ctx, states, config, failure, runtime_diagnostics) do
    diagnostics =
      states
      |> Map.values()
      |> Enum.map(&send_only_stream_diagnostic(&1, config))

    result = %{
      bytes_sent: states |> Map.values() |> Enum.map(& &1.stream_state.bytes_sent) |> Enum.sum(),
      bytes_received: 0,
      first_byte_latency_ms: nil,
      stream_latencies_ms: send_only_stream_latencies(states),
      stream_diagnostics: diagnostics,
      send_stream_call_durations_us: active_send_call_durations_us(states),
      runtime_diagnostics: runtime_diagnostics,
      active_send_duration_ms: active_send_duration_ms(states),
      active_echo_receive_duration_ms: nil,
      failure: failure
    }
  ...
  defp send_only_stream_diagnostic(state, config) do
    phase =
      cond do
        state.failure -> "send_failed"
        state.payloads_completed >= config.payload_count -> "send_only_complete"
        true -> "send_only_pending"
      end
  ...
  defp send_only_stream_latencies(states) do
    states
    |> Map.values()
    |> Enum.map(fn state ->
      finished_at = state.last_send_accepted_at || monotonic_us()
      (finished_at - state.stream_state.started_at) / 1000
    end)
  end
  ...
  defp timeout_send_only_failure(states, config) do
    state =
      states
      |> Map.values()
    ...
    if state, do: send_only_stream_failure(state, config, "send_completion_timeout")
  end
  ...
  defp send_only_receive_event_failure(states, config, reason) do
    state =
      states
      |> Map.values()
      |> Enum.reject(fn state ->
    ...
    if state, do: send_only_stream_failure(state, config, reason_name(reason))
  end
  ...
  defp stream_failure(_config, ctx, stream_state, expected_bytes, received, reason) do
    reason = reason_name(reason)
  ...
  defp profile_metadata(ctx) do
    ...
    %{
      ...
      "settings" => %{
        ...
        "traffic_sender" => traffic_sender(ctx.config, measurement),
  ...
  defp traffic_sender(%{topology: @moqx_client_topology}, measurement),
    do: measurement["traffic_sender"]

  defp traffic_sender(_config, _measurement), do: nil
  ...
  defp datagram_metrics(true, measurement) do
    %{
      ...
      "offered_rate_valid" => measurement["offered_rate_valid"],
