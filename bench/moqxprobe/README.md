...
process; it supports stream echo/drain and QUIC DATAGRAM echo. When
`--stats-output` is set, the server writes per-connection ingress statistics.
For DATAGRAM pressure, those records include received DATAGRAMs, echo-accepted
DATAGRAMs, `echo_queue_capacity`, and `echo_queue_max_depth`. For stream and
mixed workloads, they include accepted bidirectional/unidirectional stream
counts, completed stream counts, stream bytes received, and stream bytes
echo-accepted. Treat server-side received DATAGRAMs or stream bytes as the
publisher-path ingress signal; client echo delivery remains a stricter
round-trip diagnostic and can fail because echo return is backlogged after
ingress has already succeeded.
