defmodule TransportBench.Iperf3Baseline do
  @moduledoc false

  @script "bench/transport/scripts/iperf3_baseline.exs"
  @script_version "v1"
  ...
  defp parse(argv) do
    ...
    cond do
      ...
      true ->
        build_config(opts, argv)
    end
  end

  defp build_config(opts, argv) do
  ...
  defp default_path(config) do
    evidence_tier =
      if loopback?(config.server), do: "loopback_calibration", else: "edge_to_server"
    ...
    %{
      ...
      "client" => %{
        "host_id" => hostname(),
        "provider" => if(loopback?(config.server), do: "local", else: nil),
        "region" => nil,
        "instance_class" => nil,
        "os" => os_description(),
        "kernel" => kernel(),
        ...
        "nic_or_network_class" => if(loopback?(config.server), do: "loopback", else: nil)
      },
      "server" => %{
        "host_id" => config.server,
        "provider" => if(loopback?(config.server), do: "local", else: nil),
        "region" => nil,
        "instance_class" => nil,
        "os" => if(loopback?(config.server), do: os_description(), else: nil),
        "kernel" => if(loopback?(config.server), do: kernel(), else: nil),
        "cpu_model" => if(loopback?(config.server), do: cpu_model(), else: nil),
        "memory_bytes" => if(loopback?(config.server), do: memory_bytes(), else: nil),
        "nic_or_network_class" => if(loopback?(config.server), do: "loopback", else: nil)
      }
    }
  end
  ...
  defp metrics(ctx) do
    ...
    %{
      ...
      "goodput_bps" =>
        number(summary["bits_per_second"]) || number(received["bits_per_second"]) ||
      ...
      "send_rate_datagrams_per_second" =>
        if(ctx.step.protocol == "udp",
          do: rate(number(sent["packets"]) || packet_count, duration),
          else: nil
        ),
      "delivered_datagrams_per_second" =>
        if(ctx.step.protocol == "udp",
          do: rate(delivered_packets(packet_count, lost_count), duration),
          else: nil
        ),
      "datagram_delivery_ratio" => if(ctx.step.protocol == "udp", do: delivery_ratio, else: nil),
      "datagram_drop_count" => if(ctx.step.protocol == "udp", do: lost_count, else: nil),
  ...
  defp write_records(records, path) do
    body =
      records
      |> Enum.map(fn record -> record |> encode_json() |> IO.iodata_to_binary() end)
      |> Enum.join("\n")
  ...
  defp command_string(argv), do: Enum.join([@script | argv], " ")
  ...
  defp moqx_version do
    with {:ok, text} <- File.read("mix.exs"),
         [_, version] <- Regex.run(~r/version:\s*"([^"]+)"/, text) do
      version
    else
      _ -> nil
    end
  ...
  defp usage do
    """
    Usage:
      elixir bench/transport/scripts/iperf3_baseline.exs --server HOST [options]
...
TransportBench.Iperf3Baseline.main(System.argv())
